...
public class StartScreen extends JPanel {
	...
	private void initializeScoreMenu() {
		// The layout
				...
				scoreMenu.add(tableContainer, "cell 0 2, gapleft 5");
	...
	private void initializeScoreTable() {
		...
		creatColumns();
		sort();
		jTable.setGridColor(new Color(52, 152, 219));
	}
	/**
	...
	/**
	 * Creat Columns to Score and names table
	 */
	private void creatColumns() {
		dm=(DefaultTableModel) jTable.getModel();
		dm.addColumn("Player Name");
		dm.addColumn("Score");
		dm.addColumn("Max Level");
	}
	
	/**
	 * add the player name and the total score and max level
	 */
	public void addPlayerNameAndScore() {
		String[] data= {this.getPlayersName(),this.view.getGame().getLevel().getLevelScore()+this.view.getGame().getScore()+"",this.view.getGame().getLevel().getLevelNumber()+""};
		dm.addRow(data);
	}
	
	/**
	 * to enable sorting to the table
	 */
	private void sort() {
